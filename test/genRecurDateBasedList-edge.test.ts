...
describe('genRecurDateBasedList - edge cases', () => {
  ...
  it('without numericTimeZone: date and dateStr same moment (dateStr is stringified date), utcDate in UTC (0)', () => {
    ...
    assert.strictEqual(h, r.date.getUTCHours(), 'dateStr hour matches date (same moment)')
    assert.strictEqual(m, r.date.getUTCMinutes(), 'dateStr minute matches date (same moment)')
